...
/* ==========================================================================
   ...
   `descriptions` passam por t(); os arrays de `options`, `default` e as
   CHAVES do mapa `descriptions` (que precisam bater com os valores de
...
export const TYPES_01_DADOS_GERAIS = {
    ...
    IDENTIFICACAO: { label: t('lattes.tipo.IDENTIFICACAO.label', 'Identificação'), noEvidence: true, singleton: true, perfil: true, fields: [
        ...
        { key: 'corRaca', label: t('lattes.tipo.IDENTIFICACAO.campo.cor_raca.label', 'Cor ou raça'), type: 'select', options: ['Branca', 'Preta', 'Parda', 'Amarela', 'Indígena', 'Não desejo declarar'] },
        { key: 'sexo', label: t('lattes.tipo.IDENTIFICACAO.campo.sexo.label', 'Sexo'), type: 'select', options: ['Masculino', 'Feminino'], help: t('lattes.tipo.IDENTIFICACAO.campo.sexo.help', 'Exigido pelo Lattes na importação do XML.') },
        ...
        { key: 'pcd', label: t('lattes.tipo.IDENTIFICACAO.campo.pcd.label', 'Você é uma pessoa com Deficiência?'), type: 'select', options: ['Não', 'Sim'] },
        { key: 'deficiencias', label: t('lattes.tipo.IDENTIFICACAO.campo.deficiencias.label', 'Deficiência(s)'), type: 'checkboxes', disabledWhen: { field: 'pcd', in: ['', 'Não'] }, options: ['Auditiva', 'Física', 'Intelectual', 'Visual', 'Transtorno do Espectro Autista (TEA)', 'Múltipla'], descriptions: {
    ...
    DOCUMENTO_PESSOAL: { label: t('lattes.tipo.DOCUMENTO_PESSOAL.label', 'Documentos pessoais'), noExport: true, perfil: true, accept: 'application/pdf,image/jpeg,image/png', fields: [
        { key: 'tipoDoc', label: t('lattes.tipo.DOCUMENTO_PESSOAL.campo.tipo_doc.label', 'Tipo de documento'), type: 'select', required: true, options: [
    ...
    ENDERECO: { label: t('lattes.tipo.ENDERECO.label', 'Endereço'), singletonBy: 'tipo', perfil: true, accept: 'application/pdf,image/jpeg,image/png', fields: [
        { key: 'tipo', label: t('lattes.tipo.ENDERECO.campo.tipo.label', 'Tipo'), type: 'select', required: true, noBlankOption: true, default: 'Residencial', options: ['Residencial', 'Profissional'] },
