...
window.TabSumula = (function () {
    const { state, $, esc, toast, itemYear, sortByYear, getLocale, datebrParaExibicao } = window.AppCore;
    ...
    function sumulaCfgSectionHtml(cfg) {
        ...
        return `<section class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4">
            <h3 class="font-bold text-sm mb-2 flex items-center gap-2"><i class="fa-solid fa-id-card text-govbr-600 dark:text-unifesp-400"></i> ${esc('Súmula: Dados pessoais')}</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                <div><label class="block text-xs font-semibold mb-1" for="sumula-linkLattes">${esc('Link do Currículo Lattes')}</label>
                ...
                <div><label class="block text-xs font-semibold mb-1" for="sumula-linkWebOfScience">${esc('Link Web of Science (opcional)')}</label>
                ...
                <div><label class="block text-xs font-semibold mb-1" for="sumula-linkGoogleScholar">${esc('Link Google Scholar / MyCitation (opcional)')}</label>
            ...
            <div class="flex gap-2 mt-3">
                <button id="btnSaveSumulaCfg" class="px-3 py-2 rounded bg-govbr-600 dark:bg-unifesp-700 text-white text-sm"><i class="fa-solid fa-floppy-disk mr-1"></i> ${esc('Salvar')}</button>
            </div>
        </section>`;
    }
    ...
    function sumulaModelo(cfg) {
        ...
        const nadaDeclarar = 'NADA A DECLARAR';
        L.push('1) Formação');
    ...
    async function exportarSumula(cfg) {
        ...
        if (btn) { btn.disabled = true; btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin mr-1"></i> ${esc('Gerando…')}`; }
        try {
        ...
        panel.innerHTML = `
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4">
                <div class="flex items-center justify-between gap-2 mb-1 flex-wrap">
                    <h3 class="font-bold text-sm">${esc('Texto da Súmula Curricular')}</h3>
                    <button id="btnSumulaModeloPadrao" class="px-2 py-1 rounded border border-gray-300 dark:border-gray-600 text-xs"><i class="fa-solid fa-arrows-rotate mr-1"></i> ${esc('Preencher com modelo automático')}</button>
                </div>
                ...
                <textarea id="sumulaTexto" rows="20" placeholder="${esc('Clique em “Preencher com modelo automático” para começar…')}" class="w-full text-sm px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 font-mono leading-relaxed">${esc(state.sumula.texto || '')}</textarea>
                <p id="sumulaTextoSalvo" class="text-xs text-gray-400 mt-1 h-4"></p>
            </div>

            <div class="flex gap-2 flex-wrap mt-4">
                <button id="btnSumulaExportar" class="px-3 py-2 rounded bg-govbr-600 dark:bg-unifesp-700 text-white text-sm"><i class="fa-solid fa-file-export mr-1"></i> ${esc('Gerar Súmula Curricular (docx)')}</button>
            </div>`;
