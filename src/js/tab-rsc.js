...
window.TabRsc = (function () {
    const { state, $, $$, esc, toast, itemYear, formatarNumero } = window.AppCore;
    ...
    function rscCfgSectionHtml(cfg) {
        ...
        return `<section class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4">
            <h3 class="font-bold text-sm mb-2 flex items-center gap-2"><i class="fa-solid fa-id-card text-govbr-600 dark:text-unifesp-400"></i> ${esc('RSC: Dados pessoais')}</h3>
            ...
            <div class="flex gap-2 mt-3">
                <button id="btnSaveRscCfg" class="px-3 py-2 rounded bg-govbr-600 dark:bg-unifesp-700 text-white text-sm"><i class="fa-solid fa-floppy-disk mr-1"></i> ${esc('Salvar')}</button>
            </div>
        </section>`;
    }
    ...
    function render() {
        ...
        const listaHtml = Object.keys(grupos).sort().map(r => {
            ...
            <details class="border border-gray-200 dark:border-gray-700 rounded mb-2">
                <summary class="cursor-pointer px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-sm font-medium">${esc(LzRSC.REQUISITOS[r] || 'Sem requisito')} <span class="text-xs text-gray-500">— ${esc((pr.itens === 1 ? `${pr.itens} item` : `${pr.itens} itens`))} · ${esc((pr.criterios.size === 1 ? `${pr.criterios.size} critério` : `${pr.criterios.size} critérios`))} · <strong class="text-amber-700 dark:text-amber-400 tabular-nums">${formatarNumero(pr.pontos)} pts</strong></span></summary>
                <div class="p-2">${criteriosHtml}</div>
            </details>`;
        }).join('') || `<p class="text-sm text-gray-500 italic">${esc('Nenhum item marcado para o RSC ainda. Em Catalogar, marque “usar para RSC” na linha “Publicar”.')}</p>`;
        ...
        panel.innerHTML = `
            ...
            <div class="bg-gradient-to-br from-govbr-600 to-govbr-800 dark:from-unifesp-700 dark:to-unifesp-900 text-white rounded-lg p-4 mb-4">
                <p class="text-sm opacity-90">${esc('Nível alcançável')}</p>
                <p class="text-3xl font-bold">${esc(sim.nivelNome)}</p>
                <p class="text-sm mt-1">${esc('Incentivo à Qualificação:')} <strong>${sim.iq}%</strong></p>
                <p class="text-xs opacity-80 mt-2">${formatarNumero(sim.total)} pontos · ${sim.criteriosDistintos} critérios distintos</p>
                ${cfg.escolaridade ? `<p class="text-xs opacity-80">${esc(`Escolaridade limita a nível ${sim.capNivel}.`)}</p>` : `<p class="text-xs opacity-90">⚠ ${esc('Informe a escolaridade acima.')}</p>`}
            </div>

            <h3 class="font-bold mb-2">${esc('Itens contabilizados')}</h3>
            ${listaHtml}

            <div class="flex gap-2 flex-wrap mb-4 mt-4">
                <button id="btnRscPromptIA" class="px-3 py-2 rounded border border-gray-300 dark:border-gray-600 text-sm"><i class="fa-solid fa-wand-magic-sparkles mr-1"></i> ${esc('Gerar prompt (IA)')}</button>
            </div>

            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4">
                <div class="flex items-center justify-between gap-2 mb-1 flex-wrap">
                    <h3 class="font-bold text-sm">${esc('Memorial (texto final)')}</h3>
                    <button id="btnRscMemorialPadrao" class="px-2 py-1 rounded border border-gray-300 dark:border-gray-600 text-xs"><i class="fa-solid fa-arrows-rotate mr-1"></i> ${esc('Preencher com modelo automático')}</button>
                </div>
                ...
                <textarea id="rscMemorialTexto" rows="16" placeholder="${esc('Cole aqui o memorial gerado pela IA, ou escreva o seu…')}" class="w-full text-sm px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 font-mono leading-relaxed">${esc(state.rsc.memorialTexto || '')}</textarea>
                <p id="rscMemorialSalvo" class="text-xs text-gray-400 mt-1 h-4"></p>
            </div>

            <div class="flex gap-2 flex-wrap mt-4">
                <button id="btnRscExportar" class="px-3 py-2 rounded bg-govbr-600 dark:bg-unifesp-700 text-white text-sm"><i class="fa-solid fa-file-export mr-1"></i> ${esc('Gerar memorial, formulário e anexos')}</button>
            </div>`;
    ...
    function rscMemorial(itens, sim, cfg) {
        ...
        L.push(`Nível pleiteável (simulado): ${sim.nivelNome} — ${formatarNumero(sim.total)} pontos, ${sim.criteriosDistintos} critérios.`);
        L.push('');
        for (let r = 1; r <= 6; r++) {
            ...
            grp.forEach(i => {
                ...
                L.push(`  ${per}  Pontos: ${formatarNumero(pi.pontos)} (${pi.quantidade} × ${formatarNumero(pi.unitario)}).`);
    ...
    function rscFormularioBody(itens, sim, cfg, anexos) {
        ...
        parts.push(D.table([
            ...
            D.row([D.cell('Pontuação mínima necessária', { bold: true }), D.cell(minPontos != null ? formatarNumero(minPontos) : '—')]),
            D.row([D.cell('Pontuação total apresentada', { bold: true }), D.cell(formatarNumero(sim.total))]),
            D.row([D.cell('Quantidade de critérios específicos utilizados', { bold: true }), D.cell(String(sim.criteriosDistintos))]),
            D.row([D.cell('Pontuação total excedente (banco de pontos)', { bold: true }), D.cell(formatarNumero(excedente))]),
        ...
        parts.push(D.para([D.run('(Critério I + Critério II + Critério III + Critério IV + Critério V + Critério VI) TOTAL: ', { bold: true }), D.run(formatarNumero(sim.total))]));
        parts.push(D.para(`À vista das informações apresentadas, totalizo ${formatarNumero(sim.total)} pontos e atendo aos critérios legais e regulamentares para o nível ${sim.nivelNome} do RSC-PCCTAE. Solicito a análise pela CRSC-PCCTAE.`));
    ...
    async function exportarRsc(itens, sim, cfg) {
        ...
        try {
            ...
            if (btn) btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin mr-1"></i> ${esc('Gerando memorial e formulário…')}`;
            ...
            for (const a of anexos) {
                if (btn) btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin mr-1"></i> ${esc(`Copiando anexos… (${nAnexos}/${anexos.length})`)}`;
            ...
            const anexosSufixo = nAnexos ? (nAnexos === 1 ? ` (+ ${nAnexos} anexo numerado em "Anexos/")` : ` (+ ${nAnexos} anexos numerados em "Anexos/")`) : '';
            toast(`Memorial e formulário exportados em "${folder}/"${anexosSufixo}.`, 'ok');
