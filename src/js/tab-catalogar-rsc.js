...
/* ==========================================================================
   ...
   ========================================================================== */
const { state, $, $$, esc, normNome, formatarNumero } = window.AppCore;
    ...
    export function renderRscBlock(item) {
        ...
        function critListaHtml(filtro) {
            ...
            if (!encontrados.length) return `<p class="px-2 py-2 text-sm text-gray-500 italic">${esc('Nenhum critério encontrado.')}</p>`;
            const porReq = {};
            ...
            return Object.keys(porReq).map(label => {
                ...
                return `<div><p class="sticky top-0 px-2 py-1 text-[11px] font-semibold text-gray-500 bg-gray-50 dark:bg-gray-800">${esc(`Requisito ${label}`)}</p>${itens}</div>`;
            }).join('');
        }
        box.innerHTML = `
        <div id="rscFields" class="${rsc.conta ? '' : 'hidden'} bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded px-3 py-2 space-y-2">
            <div class="relative"><label class="block text-xs font-semibold mb-1" for="rscCritFiltro">${esc('Critério específico (Anexos I–VI do Decreto)')}</label>
                <input type="text" id="rscCritFiltro" autocomplete="off" placeholder="${esc('Digite pra buscar (ex.: prêmio, capacitação, comissão...)')}"
                ...
                <div id="rscCritLista" role="listbox" aria-label="${esc('Critérios encontrados')}" class="hidden absolute z-10 mt-1 w-full max-h-64 overflow-y-auto rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 shadow-lg"></div>
                <p class="text-[11px] text-gray-500 mt-0.5">${esc('Todos os critérios do decreto estão listados, agrupados por Requisito (I a VI). Digite acima para filtrar.')}</p></div>
            ...
            <div class="grid sm:grid-cols-2 gap-2">
                <div id="rscPapelWrap" class="hidden"><label class="block text-xs font-semibold mb-1" for="rscPapel">${esc('Papel')}</label>
                    <select id="rscPapel" class="w-full text-sm px-2 py-1.5 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900">
                        <option value="titular" ${rsc.papel !== 'substituto' ? 'selected' : ''}>${esc('Titular')}</option>
                        <option value="substituto" ${rsc.papel === 'substituto' ? 'selected' : ''}>${esc('Substituto')}</option></select></div>
                <div id="rscQtdWrap" class="hidden"><label class="block text-xs font-semibold mb-1" for="rscQtd">${esc('Quantidade')}</label>
            ...
            <div><label class="block text-xs font-semibold mb-1" for="rscJust">${esc('Justificativa (para o memorial)')}</label>
            ...
            <label class="flex items-center gap-2 text-sm"><input type="checkbox" id="rscUsado" ${rsc.jaUsado ? 'checked' : ''}> ${esc('Já utilizado em concessão anterior (não conta no saldo)')}</label>
        ...
        function recompute() {
            ...
            el.textContent = `Pontos: ${formatarNumero(pi.pontos)}  (${pi.quantidade} × ${formatarNumero(pi.unitario)} · ${crit.unidade})`;
        }
