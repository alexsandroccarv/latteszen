...
/* ==========================================================================
   ...
   i18n (preparação): todo texto de exibição passa por t() — inclusive
   trechos com marcação inline (<strong>/<a>/<i>), tratados como uma
   única unidade de tradução (mesmo padrão de outras páginas de conteúdo).
   ========================================================================== */
window.TabInicio = (function () {
    const { $, t } = window.AppCore;
    ...
    function render() {
        ...
        panel.innerHTML = `
            <div class="space-y-6 max-w-4xl">
                ...
                <section class="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <h2 class="text-lg font-bold mb-3 flex items-center gap-2"><i class="fa-solid fa-heart-pulse text-govbr-600 dark:text-unifesp-400"></i> ${t('tab_inicio.dores_titulo', 'Que dores o lattesZen busca resolver')}</h2>
                ...
                <section class="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <h2 class="text-lg font-bold mb-2 flex items-center gap-2"><i class="fa-solid fa-triangle-exclamation text-govbr-600 dark:text-unifesp-400"></i> ${t('tab_inicio.navegador_titulo', 'Importante: suporte do navegador')}</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
                        ${t('tab_inicio.navegador_intro', 'Para usar 100% dos recursos do lattesZen, o seu navegador precisa ter suporte ao salvamento local de arquivos.')}
                    </p>
                    <div class="space-y-2 text-sm">
                        <div class="flex gap-2"><i class="fa-solid fa-desktop text-govbr-600 dark:text-unifesp-400 mt-1"></i><span>${t('tab_inicio.navegador_computador', '<strong>Melhor opção (computador):</strong> use navegadores baseados em Chromium — Chromium, Google Chrome, Microsoft Edge, Brave, Opera ou Vivaldi.')}</span></div>
                        <div class="flex gap-2"><i class="fa-solid fa-mobile-screen text-govbr-600 dark:text-unifesp-400 mt-1"></i><span>${t('tab_inicio.navegador_celular', '<strong>No celular ou em outros navegadores:</strong> você precisará conectar sua conta do Google Drive para salvar os dados. Como temos poucas vagas gratuitas nessa integração, é preciso <a href="https://github.com/alexsandroccarv/lattesZen/issues" target="_blank" rel="noopener" class="underline">solicitar o cadastro previamente</a> para liberar o uso.')}</span></div>
                    </div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-3">
                        ${t('tab_inicio.navegador_futuro', 'Já estamos trabalhando em uma atualização para que o lattesZen rode perfeitamente em qualquer navegador no futuro!')}
                    </p>
                </section>

                <section class="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <h2 class="text-lg font-bold mb-2 flex items-center gap-2"><i class="fa-solid fa-scale-balanced text-govbr-600 dark:text-unifesp-400"></i> ${t('tab_inicio.software_livre_titulo', 'Software livre e em desenvolvimento')}</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        ${t('tab_inicio.software_livre_texto', 'O lattesZen é <strong>software livre</strong> (licença <a href="https://www.gnu.org/licenses/agpl-3.0.html" target="_blank" rel="noopener" class="underline">AGPLv3</a>) — o código é aberto: qualquer pessoa pode ler, usar, modificar e redistribuir. Ele ainda está <strong>em desenvolvimento</strong>: pode ter erros, mudar de comportamento, ganhar e perder funcionalidades de uma hora para outra. Faça backups com frequência (Configurações &rarr; Backup) e não deixe de <strong>reportar</strong> o que encontrar.')}
                    </p>
                </section>

                <section class="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <h2 class="text-lg font-bold mb-3 flex items-center gap-2"><i class="fa-solid fa-quote-left text-govbr-600 dark:text-unifesp-400"></i> ${t('tab_inicio.citar_titulo', 'Como citar')}</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">${t('tab_inicio.citar_intro', 'Se o lattesZen te ajudou numa pesquisa ou trabalho acadêmico, considere citá-lo:')}</p>
                    <blockquote class="text-sm border-l-4 border-govbr-300 dark:border-unifesp-700 pl-3 py-1.5 bg-white dark:bg-gray-900 rounded-r">
                        ${t('tab_inicio.citacao_referencia', 'CARVALHO, Alexsandro Cardoso. <em>LattesZen: descomplicando o currículo Acadêmico</em>. Versão 1.0.0. Santos: Github, 2016. DOI: <a href="https://zenodo.org/records/22781288" target="_blank" rel="noopener" class="underline">https://zenodo.org/records/22781288</a>.')}
                    </blockquote>
                    <button type="button" id="btnCopiarCitacao" class="mt-2 px-3 py-1.5 rounded border border-gray-300 dark:border-gray-600 text-xs hover:border-govbr-500 dark:hover:border-unifesp-400">
                        <i aria-hidden="true" class="fa-solid fa-copy mr-1"></i> ${t('tab_inicio.copiar_citacao', 'Copiar citação')}
                    </button>
                </section>

                <section class="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <h2 class="text-lg font-bold mb-3 flex items-center gap-2"><i class="fa-solid fa-hands-holding-circle text-govbr-600 dark:text-unifesp-400"></i> ${t('tab_inicio.colaborar_titulo', 'Como colaborar')}</h2>
                    <div class="grid sm:grid-cols-2 gap-3 text-sm">
                        <a href="https://github.com/alexsandroccarv/lattesZen/issues" target="_blank" rel="noopener" class="flex gap-2 hover:text-govbr-700 dark:hover:text-unifesp-400">
                            <i class="fa-solid fa-bug text-govbr-600 dark:text-unifesp-400 mt-1"></i><span>${t('tab_inicio.colaborar_bugs', '<strong>Reportando bugs:</strong> abra uma issue no repositório contando o que aconteceu.')}</span>
                        </a>
                        <a href="https://github.com/alexsandroccarv/lattesZen/issues" target="_blank" rel="noopener" class="flex gap-2 hover:text-govbr-700 dark:hover:text-unifesp-400">
                            <i class="fa-solid fa-lightbulb text-govbr-600 dark:text-unifesp-400 mt-1"></i><span>${t('tab_inicio.colaborar_funcionalidades', '<strong>Solicitando funcionalidades:</strong> sugestões e ideias também entram como issue.')}</span>
                        </a>
                        <a href="doe-um-cafe.html" class="flex gap-2 hover:text-govbr-700 dark:hover:text-unifesp-400">
                            <i class="fa-solid fa-mug-hot text-govbr-600 dark:text-unifesp-400 mt-1"></i><span>${t('tab_inicio.colaborar_cafe', '<strong>Doe um café:</strong> ajude com as despesas de infraestrutura do projeto.')}</span>
                        </a>
                        <div class="flex gap-2">
                            <i class="fa-solid fa-bullhorn text-govbr-600 dark:text-unifesp-400 mt-1"></i><span>${t('tab_inicio.colaborar_espalhar', '<strong>E, principalmente: use e comente que usa</strong> — espalhar a palavra é a colaboração mais simples de todas :-)')}</span>
                        </div>
                    </div>
                </section>
        ...
        if (btnCitacao) btnCitacao.addEventListener('click', () => {
            const texto = t('tab_inicio.citacao_texto_plano', 'CARVALHO, Alexsandro Cardoso. LattesZen: descomplicando o currículo Acadêmico. Versão 1.0.0. Santos: Github, 2016. DOI: https://zenodo.org/records/22781288.');
            navigator.clipboard.writeText(texto).then(() => {
                const original = btnCitacao.innerHTML;
                btnCitacao.innerHTML = `<i aria-hidden="true" class="fa-solid fa-check mr-1"></i> ${t('tab_inicio.copiado', 'Copiado!')}`;
