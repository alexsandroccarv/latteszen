...
// Dentro de Configurações, o mesmo bloqueio vale pra 3 das 4 páginas do
// menu lateral — só "Armazenamento" (onde mora o assistente) fica livre.
const CFG_PAGINAS_TRAVADAS = ['grp-fontes', 'grp-opcionais', 'grp-risco'];
...
test('Sem diretório configurado, as páginas "Trazer e levar dados"/"Outros recursos"/"Zona de risco" do menu lateral de Configurações ficam desabilitadas (com dica) — "Armazenamento" continua livre', async ({ page, baseUrl }) => {
    await ligarTravaDeVerdade(page);
    await seedCatalog(page, baseUrl, []);
    await page.click('[data-tab="config"]');
    await page.waitForTimeout(200);
